...
from .onto import check_onthology
...
def send_message(request):
    if request.POST:
        ...
        if request.POST.get('length', '') == '1':
            ...
            value_ontho = check_onthology(Validator(request.POST.get('user_text', '')).clean_input())
            if isinstance(value_ontho, str):
                response = value_ontho
            else:
